...
encoder_layer_dict = {} # for all attention/mlp layers store the layer num and their respective mask params

#encoder_layer_dict ends up being {layer1: [%, %...%]} an array for each layer
for key in percentage_dict.keys():
    
    # we skip the layernorm values
    if "layer_norm" in key or "shared.weight" in key or "decoder" in key or "relative_attention" in key:
        continue

    block = "block_" + key.split(".")[2]

    #If the layer # isn't in the dict, add it, otherwise, append it the existing dict
    if block in encoder_layer_dict:
        encoder_layer_dict[block].append(percentage_dict[key])
        
    else:
        encoder_layer_dict[block] = [percentage_dict[key]]
...
for key in encoder_layer_dict.keys():

    if count == 0:
        data = encoder_layer_dict[key]
        count += 1
    else:
        data = np.vstack( (data, encoder_layer_dict[key]))
...
col_labels = [i for i in encoder_layer_dict.keys()]
row_labels = ["q_proj", "k_proj", "v_proj", "o_proj", "Wi", "Wo"]
...
plt.savefig("T5/T5_encoder_heatmap.png", bbox_inches="tight")
...
total_params = 4*attention_params + 2*mlp_params
...
for key in encoder_layer_dict.keys():
    ...
    params_in_layer = 0
    for i in range(len(encoder_layer_dict[key])):
        if i <4:
            params = attention_params * encoder_layer_dict[key][i]
        else:
            params = mlp_params * encoder_layer_dict[key][i]
...
# Show the plot
plt.savefig("T5/T5_encoder_layer_heatmap.png", bbox_inches="tight")